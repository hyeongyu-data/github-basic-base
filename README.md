...
6. **릴리스** — 라벨별 자동 분류(`.github/release.yml`):
   ...
   ```
7. **(선택) 자동 PR 리뷰** — `.github/workflows/pr-review.yml`은 PR diff를 무료 LLM(Google Gemini 무료 티어)에 보내 요약 코멘트를 남깁니다. [AI Studio](https://aistudio.google.com/apikey)에서 무료 키를 발급받아 시크릿으로 등록하면 동작합니다(미등록 시 안내 코멘트만, 머지는 막지 않음):
   ```shell
